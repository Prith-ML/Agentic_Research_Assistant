...
def create_quick_actions():
    ...
    with col1:
        if st.button("🤖 AI Trends", use_container_width=True):
            return "What are the latest trends in artificial intelligence research?"
        
        if st.button("🧠 ML Methods", use_container_width=True):
            return "Explain the key machine learning methods and their applications"
        
        if st.button("📊 Data Science", use_container_width=True):
            return "What are the recent advances in data science and analytics?"
    
    with col2:
        if st.button("🔍 Computer Vision", use_container_width=True):
            return "What are the latest developments in computer vision?"
        
        if st.button("📝 NLP Advances", use_container_width=True):
            return "What are the recent advances in natural language processing?"
        
        if st.button("🎯 Research Tips", use_container_width=True):
            return "How can I effectively conduct literature reviews in AI/ML?"
